...
chcp 65001 >nul
set "BAT_DIR=%~dp0"
powershell -NoProfile -ExecutionPolicy Bypass -Command "& { $f='%~f0'; Invoke-Command -ScriptBlock ([scriptblock]::Create(((Get-Content $f -Encoding UTF8) | Select-Object -Skip 4) -join [char]10)) }"
exit /b
# =============================================================
#  НАСТРОЙКИ — измени под свой репозиторий
$Owner  = 'ChesterX'
$Repo   = 'client'
$Branch = 'main'
$Dest   = $env:BAT_DIR.TrimEnd('\')
# =============================================================
...
# --- Получаем дерево файлов через API ---
...
# --- Фаза 1: Обновление/скачивание файлов ---
...
foreach ($rf in $remoteFiles) {
    ...
    try {
        ...
        $nameColor = 'Red'
        $errors++
        if ($newf    -gt 0 -and $status -eq 'ОШИБКА   ') { $newf-- }
        if ($updated -gt 0 -and $status -eq 'ОШИБКА   ') { $updated-- }
    }
